#!/usr/bin/env python3
"""
Modal training entrypoint for the RLAD noise-control Hendrycks MATH experiment.
...
app = modal.App("rlad-hendrycks-train")
...
REPO_PATH = "/root/e3"
DATA_DIR = "/data/hendrycks_math"          # parquets live here on the Volume
...
def run_train(track: str, total_steps: int, save_freq: int, test_freq: int):
    ...
    for p in (train_parquet, val_parquet):
        if not os.path.exists(p):
            raise FileNotFoundError(
                f"Missing parquet on Volume: {p}. Did you run scripts/data_upload_hendrycks.py?"
            )
    ...
    env.update({
        ...
        "WANDB_PROJECT": "rlad-hendrycks",
    })

    cmd = ["bash", os.path.join(REPO_PATH, "scripts/grpo/grpo_hendrycks_a100.sh")]
